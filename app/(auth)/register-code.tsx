...
export default function RegisterCodeScreen() {
  ...
  const { account, accountType, setCode: setCodeStore } = useRegisterFlowStore();
  ...
  const handleResend = async () => {
    if (!accountType || !account) { router.back(); return; }
    if (countdown > 0) return;
    try {
      await authApi.sendCode({ accountType, account, codeType: 1 });
      setCountdown(60);
...
const styles = StyleSheet.create({
  ...
  primaryButton: {
    backgroundColor: "#000000", borderRadius: 14, paddingVertical: 17, alignItems: "center",
    shadowColor: "#000000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.15, shadowRadius: 12, elevation: 4,
  },
